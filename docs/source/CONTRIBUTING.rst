...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    bugs to the issue tracker, or elsewhere in public. Instead sensitive
    bugs must be sent by email to screeps@pieterbrandsen.nl.
...
-  If the team is able to reproduce the issue, it will be marked
   ...
   someone <#your-first-code-contribution>`__.
...
You are welcome to contribute, fork the project, do you changes and open a pull request.
...
Interested? Join the `Screeps slack <https://screeps.slack.com/>` and send me a message at **@pandamaster**.
