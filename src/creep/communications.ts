...
export default class CreepCommunication {
  public static GetCurrentJob = FuncWrapper(function CreepGetCurrentJob(
    creepName: string
  ): FunctionReturn {
    try {
      ...
      const job = JobHandler.GetJob(creepMemory.jobId, creepMemory.commandRoom);
      if (job === null) {
        return FunctionReturnHelper(
          GlobalConstants.FunctionReturnCodes.NOT_FOUND
        );
      ...
      return FunctionReturnHelper(GlobalConstants.FunctionReturnCodes.OK, job);
    } catch (error) {
      return FunctionReturnHelper(
